...
const VLink = ({ baseurl, href, label, icon }: VLinkProps) => {
    ...
    return (
        <Link 
          href={fullPath} 
          aria-label={label} 
          className={`w-full p-2 m-2 text-3xl  hover:text-amber-500 ${isActive ? "text-amber-500 border-r-4 border-amber-500 bg-gradient-to-l" : "text-black border-0"} transition-all duration-150`}
        >
          {icon}
        </Link>
    )
